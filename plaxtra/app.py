...
init_db()
app=FastAPI(title='Plaxtra',version='0.3.0',docs_url='/api/docs',redoc_url='/api/redoc')
...
def watch(): return shell('Player','''<div class="player"><video id="video" controls autoplay playsinline></video><h1>Plaxtra Player</h1></div><script>const url=new URLSearchParams(location.search).get('url')||'';const v=document.getElementById('video');if(window.Hls&&Hls.isSupported()){const h=new Hls();h.loadSource(url);h.attachMedia(v)}else v.src=url</script><script src="https://cdn.jsdelivr.net/npm/hls.js@1.5.17/dist/hls.min.js"></script>''')
